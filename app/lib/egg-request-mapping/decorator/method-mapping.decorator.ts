...
const createHttpMethodDecorator = (method: string) => (path = ''): MethodDecorator => {
  return (target, key) => {
    ...
    Reflect.defineMetadata(CONTROLLER_METHOD_METADATA, method, target, key);
    // 设置控制器里的路由地址
    Reflect.defineMetadata(CONTROLLER_ROUTE_METADATA, path, target, key);
  };
};

export function Control (prefix = '') {
  const strArr: string[] = [];
  if (prefix.length > 0) {
